...
class Scale(object):
    ...
    def __call__(self, inputs):
        ...
        inputs[0] = resize(inputs[0], self.size)
        inputs[1] = resize(inputs[1], self.size)
...
class RandomRotate(object):
    ...
    def __call__(self, inputs):
        ...
        inputs[1] = ndimage.interpolation.rotate(inputs[1], angle1, reshape=self.reshape, order=self.order)
